...
export function openRegistryStore(dbPath: string): RegistryStorePort {
  ...
  `);
  // 旧库迁移：缺列补列（trusted NULL = 未记录按非受信；keepalive 0 = 不豁免回收）
  const columns = db.prepare('PRAGMA table_info(sessions)').all() as Array<{ name: string }>;
  if (!columns.some((column) => column.name === 'trusted')) {
    db.exec('ALTER TABLE sessions ADD COLUMN trusted INTEGER');
  }
  if (!columns.some((column) => column.name === 'keepalive')) {
    db.exec('ALTER TABLE sessions ADD COLUMN keepalive INTEGER NOT NULL DEFAULT 0');
  }
  ...
  return {
    list(): SessionRow[] {
      const result = db.prepare('SELECT threadId, sessionPath, cwd, title, trusted, createdAt, updatedAt, keepalive FROM sessions ORDER BY updatedAt DESC').all();
      return result
    ...
    upsert(row: SessionRow): void {
      db.prepare(
        `INSERT INTO sessions (threadId, sessionPath, cwd, title, trusted, createdAt, updatedAt, keepalive)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ...
      ).run(row.threadId, row.sessionPath, row.cwd, row.title, row.trusted === null ? null : row.trusted ? 1 : 0, row.createdAt, row.updatedAt, row.keepalive ? 1 : 0);
    },
    get(threadId: string): SessionRow | null {
      const row = db.prepare('SELECT threadId, sessionPath, cwd, title, trusted, createdAt, updatedAt, keepalive FROM sessions WHERE threadId = ?').get(threadId);
