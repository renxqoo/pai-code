...
/**
 * v0.14 收敛读口视图（get_inflight / get_subagents / get_pending_dialogs）。
...
/** 在途 assistant 消息（尚未落盘的那半段）：与 message_end 同源形状的正规化视图。 */
...
export const InflightViewSchema = z.object({
  /** 本轮持久前缀边界（agent_start 时刻的 leaf 条目 id）；null = 无在途轮。 */
  turnStartEntryId: z.string().nullable(),
...
/** get_subagents 的视图（registry 快照；无子代理为空数组）。 */
