 ...
 * 位置与宽度由调用方决定，容器只负责外观、标题栏与内容区内滚。
 */
function FloatingPanel({ label, title, actions, maxContentHeight, children, className }: FloatingPanelProps) {
  ...
  return (
    <section aria-label={label} className={cn(panelSurfaceClassName, 'flex flex-col overflow-hidden', className)}>
      ...
      <div style={contentStyle} className="min-w-0 overflow-y-auto overscroll-contain">
        {children}
      </div>
    </section>
  )
