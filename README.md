...
一款 AI 助手桌面应用（macOS）。你可以同时开多个对话，让 AI 助手真正动手干活——查资料、改代码、执行任务，还能派出多个子助手并行处理。

- **先问后做**：助手执行敏感操作（如运行命令、修改文件）前会先征求你的同意，你确认后才执行
- **一目了然**：任务进度、分支图谱、用量统计随时可查
- **各司其职**：应用分两部分——你看到的窗口只负责展示和交互；真正干活的是后台一个独立的「引擎」（pi-hub），负责运行 AI、管理会话、保管密钥。窗口和引擎之间通过一条专用消息通道通信
