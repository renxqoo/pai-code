...
Electron 桌面应用：通过**单个 pi-hub 宿主进程**承载多对话 × 多线程的 AI Agent。

职责边界一句话：**hub 执行，Pai 展示**。hub（独立仓库）承担 Agent 执行、会话、权限判定与拦截、OS 沙箱、凭据存储；Pai 负责 hub 进程管理与全部展示交互（对话、权限审批、分支图谱、用量监控等），操作一律经协议提交。两者之间的唯一交互面是 JSONL stdio 协议：1Hz 心跳、对话框 `ui_request`/`ui_response`、权限规则命令。
