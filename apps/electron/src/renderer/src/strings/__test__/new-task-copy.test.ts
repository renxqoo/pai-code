...
describe('newTask.branchFailed', () => {
  test.each(['branch_exists', 'invalid_branch', 'dirty_worktree', 'unknown_branch', 'not_a_repo', 'git_unavailable', 'cwd_not_found'])(
    ...
    (reason: string) => {
      expect(copy.newTask.branchFailed(reason)).not.toContain(reason);
    },
  );

  test('未知 reason 透传原因（排障可读，不吞）', () => {
    expect(copy.newTask.branchFailed('weird_state')).toContain('weird_state');
  });
});
