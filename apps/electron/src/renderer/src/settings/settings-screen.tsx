 ...
 */
function SettingsScreen({ open, providers, saved, onClose, onUpsertProvider, onRemoveProvider, onOpenSaved, onRefreshSaved }: SettingsScreenProps) {
  const [section, setSection] = React.useState<'providers' | 'history'>('providers');
  if (!open) return null;
  const navItems = [
