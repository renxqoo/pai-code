/** 技能导入面板内容（自 skill-import-dialog 拆出——一组件一文件纪律）：
 *  候选列表（勾选多选 + 目标名可改 + 三态徽章 + 诊断）+ 覆盖同名显式确认 +
 *  执行汇总（逐条失败明细）。与 Dialog 外壳分离 = 可 SSR 测试（Base UI Popup 走
...
export function SkillImportContent({
  candidates,
  installedNames,
  selections,
  busy,
