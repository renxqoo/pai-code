...
/**
 * 历史轮左缘锚点：常显轻量刻痕，悬停/聚焦浮出「时刻 + 摘要」气泡，
 * 点击把该轮滚入视口。挂在轮次容器左外的栏沟里；窄视口（<1280px）无栏沟，
 * 整体隐藏而不是留下半个被裁切的标记。
 */
function TurnAnchor({ time, summary, onJump }: TurnAnchorProps) {
  return (
    <div className="absolute top-[1px] right-full mr-[16px] hidden min-[1280px]:block">
      <button
        type="button"
        aria-label={copy.flow.turnAnchorAria(time)}
        onClick={onJump}
        className="group relative flex h-[20px] w-[22px] cursor-pointer items-center justify-center rounded-md outline-none select-none focus-visible:ring-3 focus-visible:ring-ring/50"
      >
        <span
          aria-hidden="true"
          className="h-[3px] w-[14px] rounded-full bg-border transition-colors duration-150 group-hover:bg-foreground/50 group-focus-visible:bg-foreground/50 motion-reduce:transition-none"
        />
        <span
          aria-hidden="true"
