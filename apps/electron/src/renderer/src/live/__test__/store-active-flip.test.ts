...
/**
 * 活跃线程翻转的会话级视图失效（T33 M1 审查 #2 回归）：sessionRules/commands/
 * effortLevels 必须随 activeThreadId 的全部翻转路径同步清空——直写 activeThreadId
 * 不带补丁的路径会让被移除会话的补全目录无限残留。
 */
...
describe('活跃线程翻转清空会话级视图', () => {
  test('setActiveThread：切会话同清 commands/effortLevels/sessionRules；同值重设不清', () => {
    store.getState().applyEvent(updated('a'), 0);
    store.getState().setActiveThread('a'); // 先成为活跃线程（null→a 本身是翻转）
    store.setState({ commands: [{ command: '/x', description: '' } as never], effortLevels: ['high'] });
    ...
    expect(s.effortLevels).toEqual([]);
    expect(s.sessionRules).toBe(null);
  });
  ...
  test('sessionRemoved：移除活跃会话回落继任者/首会话时同清（不再残留被移除会话的目录）', () => {
    ...
    store.getState().setActiveThread('a');
    store.setState({ commands: [{ command: '/x', description: '' } as never], effortLevels: ['high'], sessionRules: { rules: { mode: 'ask' }, source: 'global' } });
  ...
  test('sessionRemoved：移除非活跃会话不清（活跃未翻转）', () => {
    ...
    store.getState().applyEvent(updated('b'), 0);
    store.getState().setActiveThread('a');
    store.setState({ commands: [{ command: '/x', description: '' } as never] });
