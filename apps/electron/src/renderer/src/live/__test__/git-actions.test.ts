...
describe('searchFiles', () => {
  ...
  test('成功透传列表；失败返回 null（弹层按空结果呈现）', async () => {
    ...
    const bad = makeClient({ 'file/search': { ok: false, reason: 'cwd_forbidden' } });
    expect(await searchFiles(bad.client, '/w', 'a')).toBeNull();
  });
});

describe('listGitBranches', () => {
  test('空 cwd 返回 cwd_not_allowed（与主进程门禁同 reason，调用方无需分支）', async () => {
    const { client, calls } = makeClient({});
    expect(await listGitBranches(client, '')).toEqual({ ok: false, reason: 'cwd_not_allowed' });
    expect(calls).toEqual([]);
  });
  ...
  test('非仓库空形态与失败面原样透传', async () => {
    ...
    const failed = makeClient({ 'git/branches': { ok: false, reason: 'git_unavailable' } });
    expect(await listGitBranches(failed.client, '/w')).toEqual({ ok: false, reason: 'git_unavailable' });
  });
});
...
describe('checkoutGitBranch', () => {
  test('空 cwd 早退；参数原样提交（create 透传）', async () => {
    ...
    expect(await checkoutGitBranch(client, '', 'dev', false)).toEqual({ ok: false, reason: 'cwd_not_allowed' });
  ...
  test('失败原因透传（脏工作区/分支已存在）', async () => {
    const dirty = makeClient({ 'git/checkout': { ok: false, reason: 'dirty_worktree' } });
    expect(await checkoutGitBranch(dirty.client, '/w', 'dev', false)).toEqual({ ok: false, reason: 'dirty_worktree' });
    const dup = makeClient({ 'git/checkout': { ok: false, reason: 'branch_exists' } });
    expect(await checkoutGitBranch(dup.client, '/w', 'dev', true)).toEqual({ ok: false, reason: 'branch_exists' });
  });
});
