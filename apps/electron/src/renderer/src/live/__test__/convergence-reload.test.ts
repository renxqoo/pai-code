...
import type { InflightView, SessionView } from '@paiapp/contracts';
...
function bootStore(sessions: SessionView[]): LiveStore {
  ...
  store.getState().bootstrap(bootstrapOf(sessions).ok ? (bootstrapOf(sessions) as { ok: true; data: unknown }).data as never : ({} as never));
  return store;
}
...
describe('applyInflight · 在途快照合入（单条消息重载，M2b）', () => {
  test('症状回归「单条消息流式中刷新只剩半截」：快照补回重载前已流出的正文，取更长者', () => {
    ...
    state = applyInflight(
      state,
      inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: '前半后半', thinking: '', toolCalls: [] } }),
      1_000,
    );
  ...
  test('工具在途输出：替换 live 的累积输出、status 归 running、时长由 startedAt 复原', () => {
    const withCall = applyInflight(
      { ...initialThreadState, streaming: true },
      inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: '跑工具', thinking: '', toolCalls: [{ id: 'c1', name: 'bash', argsPreview: 'bun test' }] } }),
      1_000,
    );
    const merged = applyInflight(
      withCall,
      inflightView({ turnStartEntryId: 'e1', toolOutputs: [{ callId: 'c1', output: '3 pass', truncated: false, startedAt: 500 }] }),
      1_500,
    );
  ...
  test('bash 在途：横幅可重建（bashRunning + 尾部输出）', () => {
    const state = applyInflight(
      { ...initialThreadState },
      inflightView({ bash: { command: 'pytest', output: 'collected 3 items\n', truncated: false, startedAt: 900 } }),
      1_000,
    );
    expect(state.bashRunning).toBe(true);
    expect(state.bashTail).toBe('collected 3 items\n');
    expect(state.turnStartEntryId).toBeNull();
  });

  test('空形态（无在途轮）不改变视图，只更新轮起点事实', () => {
    const base: LiveThreadState = { ...initialThreadState, turnStartEntryId: 'e9' };
  ...
  test('轮起点非空 = 轮在途：streaming 置真（重载后未收到任何事件时也不把新消息误判为空闲）', () => {
    const state = applyInflight({ ...initialThreadState }, inflightView({ turnStartEntryId: 'e1' }), 1_000);
    expect(state.streaming).toBe(true);
    expect(state.turnStartEntryId).toBe('e1');
  });
});
...
describe('重载收敛链（读序 + 降级，M2b）', () => {
  test('症状回归「单条消息流式中刷新」：in-flight 读带回在途正文，视图不再只剩增量', async () => {
    ...
    const client = makeClient((method) => {
      ...
      if (method === 'session/inflight')
        return { ok: true, data: inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: '重载前已流出的正文', thinking: '', toolCalls: [] } }) };
  ...
  test('读序规则：in-flight 先于 entries（恰在两读之间落盘的消息由后读的 entries 兜住）', async () => {
    ...
    const persisted = { kind: 'assistant', id: 'e9', messageTs: 9, text: '刚落盘的一条', thinking: '', toolCalls: [], usage: null, stopReason: null, errorMessage: null, at: 2 };
    const client = makeClient((method) => {
      if (method === 'app/bootstrap') return bootstrapOf(sessions);
      if (method === 'session/inflight') {
        inflightRead = true;
        return { ok: true, data: inflightView({ turnStartEntryId: 'e9' }) };
      }
      if (method === 'session/entries') {
        // 该消息恰在 inflight 读之后、entries 读之前落盘：先读 entries 就永远看不到它
        return { ok: true, data: { items: inflightRead ? [persisted] : [], cursor: 'e9' } };
      }
      return { ok: true, data: null };
    });
    const store = bootStore(sessions);
  ...
  test('读口不可用（旧 hub：命令 failure）→ 跳过且不崩，entries 仍按既有路径水化', async () => {
    const sessions = [sessionView('t1', 'live', '/w/s/t1.jsonl')];
    const persisted = { kind: 'assistant', id: 'a1', messageTs: 1, text: '已落盘', thinking: '', toolCalls: [], usage: null, stopReason: null, errorMessage: null, at: 2 };
    const client = makeClient((method) => {
      if (method === 'app/bootstrap') return bootstrapOf(sessions);
      ...
      if (method === 'session/entries') return { ok: true, data: { items: [persisted], cursor: 'a1' } };
      return { ok: true, data: null };
    });
    const store = bootStore(sessions);
    store.getState().setActiveThread('t1');
  ...
  test('症状回归「结算后 host 报空在途面」：不建轮、streaming 不滞留（守卫与空形态各司其职）', async () => {
    ...
    client.emitToController({ type: 'turnSettled', threadId: 't1', usage: null });
    await controller.ensureHydrated('t1');
    await waitMs(0);
  ...
  test('症状回归「重载后直接执行的 bash 结果条目不出现、横幅常亮」（fb-8）：输出静默后用读口收尾并补拉条目', async () => {
    const sessions = [sessionView('t1', 'live', '/w/s/t1.jsonl')];
    const persisted = { kind: 'bash', id: 'b1', command: 'bun test', output: '3 pass', exitCode: 0, cancelled: false, at: 5 };
    ...
    const client = makeClient((method) => {
      ...
      if (method === 'session/inflight')
        return {
          ok: true,
          data: inflightView(bashRunning ? { bash: { command: 'bun test', output: 'partial', truncated: false, startedAt: 1 } } : {}),
        };
      if (method === 'session/entries') return { ok: true, data: { items: [persisted], cursor: 'b1' } };
      return { ok: true, data: null };
    });
    const store = bootStore(sessions);
    store.getState().setActiveThread('t1');
    ...
    client.emitToController({ type: 'bashOutput', threadId: 't1', delta: '3 pass' });
  ...
  test('症状回归「子代理在途弹窗重载后丢失」：读口带回的 grandchild 弹窗按实时帧同口径重建（subagentId/agent 归位）', async () => {
    const sessions = [sessionView('t1', 'live', '/w/s/t1.jsonl')];
    const client = makeClient((method) => {
      if (method === 'app/bootstrap') return bootstrapOf(sessions);
      if (method === 'session/inflight') return { ok: true, data: inflightView({}) };
            ...
            dialogs: [
              {
                ...
                threadId: 't1',
                method: 'confirm',
                payload: { method: 'confirm', title: 'Allow command execution?', message: 'echo hi', subagentId: 'sub_1', agent: 'explore' },
              },
            ],
          },
        };
...
describe('对抗审查补强（T35 复审）', () => {
  test('症状回归「轮内注入 user 消息把同一轮切成两个」：span 归属按读口边界（turnStartEntryId）而非末位用户消息', () => {
    // 转写：上轮末条目 e0 → 本轮 m1(assistant) → 轮内注入 user(e2)
    ...
    const derived = [
      history({ kind: 'assistant', id: 'e1', messageTs: 1001, text: 'm1权威' }),
      history({ kind: 'user', id: 'e2', text: '[task-notification] 注入' }),
    ] as never;
    state = foldHydrate(state, { kind: 'hydrate/inflight', view: inflightView({ turnStartEntryId: 'e0' }), at: tick(3) });
    state = foldHydrate(state, { kind: 'hydrate/reconcile', items: derived, cursor: 'e2', dropLiveTurn: false });
  ...
  test('症状回归「快照新于已收增量时正文重复」：快照是 live 的后缀 → 取快照', () => {
    const withDelta = applyInflight({ ...initialThreadState, streaming: true }, inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: 'B', thinking: '', toolCalls: [] } }), 1_000);
    const merged = applyInflight(withDelta, inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: 'AB', thinking: '', toolCalls: [] } }), 1_100);
  ...
  test('症状回归「工具执行中刷新：工具卡假完成」：无在途消息也建轮，且转写说完成/读口说在跑 → running', () => {
    ...
    let state = applyInflight(
      initialThreadState,
      inflightView({ turnStartEntryId: 'e0', toolOutputs: [{ callId: 'c1', output: 'line1', truncated: false, startedAt: 900 }] }),
      1_000,
    );
    expect(state.liveTurnId).not.toBeNull();
    // 转写随后带回该 assistant（含工具调用，无 toolResult）——不得插成独立历史轮
    const derived = [history({ kind: 'assistant', id: 'e1', messageTs: 1001, text: '', toolCalls: [{ id: 'c1', name: 'bash', argsPreview: 'bun test', output: '', isError: false, diff: null }] })] as never;
    state = foldHydrate(state, { kind: 'hydrate/reconcile', items: derived, cursor: 'e1', dropLiveTurn: false });
    const turns = state.items.filter((item) => item.kind === 'turn');
    expect(turns).toHaveLength(1);
  ...
  test('症状回归「读在途期间结算 → 迟到快照不得重新点亮该轮」', async () => {
    ...
    client.emitToController({ type: 'turnSettled', threadId: 't1', usage: null });
    release({ ok: true, data: inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: '过期在途', thinking: '', toolCalls: [] } }) });
    await hydrating;
    await waitMs(0);
  ...
  test('症状回归「轮在途但读口空在途面（message_end 与落盘之间的窗口）」：复拉一次 entries 兜底', async () => {
    const sessions = [sessionView('t1', 'live', '/w/s/t1.jsonl')];
    const persisted = { kind: 'assistant', id: 'e2', messageTs: 9, text: '刚落盘的一条', thinking: '', toolCalls: [], usage: null, stopReason: null, errorMessage: null, at: 2 };
    let entriesCalls = 0;
    const client = makeClient((method) => {
      if (method === 'app/bootstrap') return bootstrapOf(sessions);
      if (method === 'session/inflight') return { ok: true, data: inflightView({ turnStartEntryId: 'e1' }) };
      if (method === 'session/entries') {
        entriesCalls += 1;
        // 首读落在两步窗口内（条目尚未可见），复拉时才可见
        return { ok: true, data: { items: entriesCalls > 1 ? [persisted] : [], cursor: 'e2' } };
      }
      return { ok: true, data: null };
    });
    const store = bootStore(sessions);
...
describe('用户现场回归（2026-09-11 夜：多次刷新后思考只剩碎片 + 计时从 0 开始）', () => {
  test('症状回归「刷新后思考只剩刷新后的碎片」：快照全文与 live 尾段合并后必须是完整思考', () => {
    ...
    const withDelta = applyInflight(
      { ...initialThreadState, streaming: true },
      inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: '', thinking: 'to the', toolCalls: [] } }),
      1_000,
    );
    const merged = applyInflight(
      withDelta,
      inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: '', thinking: 'I need to check the file to the', toolCalls: [] } }),
      1_100,
    );
  ...
  test('症状回归「上一轮结算事件先到，当前轮的在途快照被误丢」：快照只在读取期间结算才丢弃', async () => {
    ...
    const client = makeClient((method) => {
      if (method === 'app/bootstrap') return bootstrapOf(sessions);
      if (method === 'session/inflight')
        return { ok: true, data: inflightView({ turnStartEntryId: 'e1', message: { messageTs: 7, text: '本轮在途正文', thinking: '', toolCalls: [] } }) };
      if (method === 'session/entries') return { ok: true, data: { items: [], cursor: null } };
      return { ok: true, data: null };
    });
    const store = bootStore(sessions);
  ...
  test('老 hub 兜底：读口不给 turnStartedAt 时，live 轮计时锚点回填为本轮 prompt 时刻', () => {
    let state = applyInflight(
      { ...initialThreadState, streaming: true },
      inflightView({ turnStartEntryId: 'e0' }),
      1_000_000,
    );
    state = foldHydrate(state, {
      kind: 'hydrate/reconcile',
      items: [history({ kind: 'user', id: 'u9', text: '本轮问', at: 400_000 })],
      cursor: 'u9',
      dropLiveTurn: false,
    });
  ...
  test('症状回归「轮计时从 0 开始」：live 轮的 startedAt 取轮首事实（不是刷新时刻）', () => {
    const withTurn = applyInflight(
      { ...initialThreadState, streaming: true },
      inflightView({ turnStartEntryId: 'e1', turnStartedAt: 500_000, message: { messageTs: 7, text: 'x', thinking: '', toolCalls: [] } }),
      1_000_000,
    );
