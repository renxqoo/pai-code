...
export function useEscDismiss(input: UseEscDismissInput): void {
  const { dialogCount, sidebarSearchOpen, usageOpen, projectFilesOpen, newTaskOpen, settingsOpen, panelOpen, bashRunning, confirmStop, generating, agentsActive } = input;
  ...
  React.useEffect(() => {
    const onKeyDown = (event: KeyboardEvent): void => {
      ...
      const action = escActionFor({ dialogCount, sidebarSearchOpen, usageOpen, projectFilesOpen, newTaskOpen, settingsOpen, panelOpen, bashRunning, confirmStop, generating, agentsActive });
      switch (action.kind) {
  ...
  }, [dialogCount, sidebarSearchOpen, usageOpen, projectFilesOpen, newTaskOpen, settingsOpen, panelOpen, bashRunning, confirmStop, generating, agentsActive, abortBash, stopActiveTurn, onSidebarSearchClose, onUsageClose, onProjectFilesClose, onNewTaskClose, onSettingsClose, onPanelClose, onConfirmStopChange]);
}
