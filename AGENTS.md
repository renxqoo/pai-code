...
- **权限与沙箱**：权限判定、拦截、规则存储、超时默认拒绝、OS 沙箱**全部在 hub**；Pai 只做 UI 交互并把操作经协议提交（`ui_response` 回包、rules 命令、`thread/start` 的 trusted 参数），不写规则文件、不镜像判定逻辑、不经手 spawn 包裹；thread 默认 `trusted:false` 不加载项目扩展
